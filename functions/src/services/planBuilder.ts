...
const EXERCISES_PER_BODY_PART: Record<BodyPart, number> = {
  chest: 3,
  back: 3,
  shoulders: 2,
  legs: 4,
 ...
 */
function buildSeedKey(uid: string, profile: NormalizedProfile, now: Date): string {
...
function buildRecovery(profile: NormalizedProfile): string[] {
  ...
  if (profile.limitations.length > 0) {
    notes.push(
      "You reported a physical limitation — stop any exercise that causes sharp or worsening pain and consult a medical professional if it persists."
    );
  }
  return notes;
}

function buildGuidelines(profile: NormalizedProfile): string[] {
  const strategy = getGoalStrategy(profile.goal);
  const guidelines = [...strategy.guidelines];
  if (profile.limitations.length > 0) {
    guidelines.push(
      `Exercises that could aggravate your reported limitation(s) (${profile.limitations.join(
        ", "
      )}) have been excluded from this plan, with safer alternatives substituted where possible.`
    );
  }
  return guidelines;
}

function buildNutrition(profile: NormalizedProfile) {
  const strategy = getGoalStrategy(profile.goal);
...
export function buildPlan(uid: string, profile: NormalizedProfile, now: Date = new Date(), weekNumber = 1, previousPlanId: string | null = null): GeneratedPlan {
  ...
  const dailyWorkouts: Record<string, PlanExercise[]> = {};
  const weeklySplitLabels: string[] = [];
